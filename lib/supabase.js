...
export const DatabaseService = {
  ...
  // User Exam Preferences Operations
  async getUserExamPreferences(userId) {
    try {
      console.log('Getting user exam preferences for:', userId);
      ...
      if (error && error.code !== 'PGRST116') {
        console.error('Error getting user exam preferences:', error);
        throw error;
      }

      if (!data) {
        console.log('No exam preferences found, returning defaults');
        return {
          tyt_enabled: false,
      ...
      console.log('User exam preferences loaded successfully');
      return data;
    } catch (error) {
      console.error('Error getting user exam preferences:', error);
      throw error;
  ...
  async updateUserExamPreferences(userId, preferences) {
    try {
      console.log('Updating user exam preferences for:', userId);
      ...
      console.log('User exam preferences updated successfully');
      return data;
    } catch (error) {
      console.error('Error updating user exam preferences:', error);
      throw error;
  ...
  async getEmailVerificationStatus(userId) {
    try {
      console.log('Getting email verification status for:', userId);
      
      const { data, error } = await supabase
        .from('email_verification')
        .select('verified_at, verification_code, expires_at')
        .eq('user_id', userId)
        .single();
      ...
      console.log('Email verification status loaded successfully');
      return data;
    } catch (error) {
      console.error('Error getting email verification status:', error);
      throw error;
  ...
  async sendVerificationCode(userId, email) {
    try {
      console.log('Sending verification code for user:', userId);
  ...
  async isEmailVerified(userId) {
    try {
      console.log('Checking if email is verified for user:', userId);
      ...
      console.log('Email verification check result:', data);
      return data;
    } catch (error) {
      console.error('Error checking email verification:', error);
      throw error;
  ...
  async hasExamPreferences(userId) {
    try {
      console.log('Checking if user has exam preferences:', userId);
      
      const { data, error } = await supabase.rpc('has_exam_preferences', {
        user_id: userId
      });

      if (error) {
        console.error('Error checking exam preferences:', error);
        throw error;
      }

      console.log('Exam preferences check result:', data);
      return data;
    } catch (error) {
      console.error('Error checking exam preferences:', error);
      throw error;
